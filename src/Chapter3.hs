...
fight k m
  | (monsterHealth monsterTakesHit) <= 0 = knightGold k + monsterGold m
  | (knightHealth knightTakesHit) <= 0 = (-1)
  | otherwise = knightGold k
  where
    monsterTakesHit = m {monsterHealth = (monsterHealth m - knightHealth k)}
    knightTakesHit = k {knightHealth = (knightHealth k - monsterHealth m)}
